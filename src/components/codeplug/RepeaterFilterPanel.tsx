...
export function RepeaterFilterPanel({
  summary,
  settings,
  setSettings,
}: {
  ...
  const allTypes = Object.keys(summary.uniqueCounts.type).filter((t) => !OUT_OF_SCOPE_TYPES.has(t));
  ...
  const regionOptions = Object.keys(presentRegions).filter(Boolean).sort();

  const selectedCountries = settings.filter.countries ?? [];
  const selectedRegions = settings.filter.regions ?? [];

  const setNordicQuick = () => upd({ countries: [...NORDIC_COUNTRY_CODES], regions: [] });
  const clearCountries = () => upd({ countries: [], regions: [] });
  const setSwedenOnly = () => upd({ countries: ["SE"], regions: [] });
    ...
    <div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <MultiSelect
          label="Status"
          options={allStatuses}
          value={settings.filter.statuses}
          onChange={(v) => upd({ statuses: v })}
        />
        <MultiSelect
          label="Typ"
          options={allTypes}
          value={settings.filter.types}
      ...
      <div className="mt-3">
        ...
        />
        <Hint>
          Visar regioner som finns i den importerade filen (SM0–SM7, LA, OZ, OH0–OH9, TF, JW, JX,
          OY, OX).
        </Hint>
        <label className="mt-2 flex items-center gap-2 text-sm">
          <input
            type="checkbox"
            checked={settings.filter.includeUnknownRegions ?? false}
      ...
      <div className="mt-4">
        ...
        <Hint>
          Markera de modes som ska exporteras. En SK6BA-rad med flera modes (t.ex. "FM / C4FM")
          expanderar till en kanal per markerat mode. Modes som inte stöds av valt exportformat är
          utgråade.
        </Hint>
        <ModeToggles
...
function ChirpDigitalNote({
  ...
  return (
    <p className="mt-2 rounded-md border border-amber-400/40 bg-amber-50 px-3 py-2 text-xs text-amber-900 dark:border-amber-500/30 dark:bg-amber-950/40 dark:text-amber-200">
      CHIRP Generic CSV kan bära digitala mode-värden (DN, DV, DMR, P25), men fullt stöd beror på
      radiomodell och CHIRP-drivrutin. Systemspecifika inställningar som DMR talkgroup, color code,
      timeslot eller Fusion- parametrar ingår inte och kan behöva kompletteras manuellt.
    </p>
  );
}
